	    		     ...
	    		     int (*link_selector) (int, int, DocInfo *, int *) )
{
    ...
    //switch (lazylynx_follow_link_click(c, lindx, &newdoc, &number)) {
    //switch (lazylynx_follow_link_select(c, lindx, &newdoc, &number)) {
...
int mainloop(void)
{
  ...
  initialize:
	...
	case LYK_LAZY_LINK_CLICK:
	    handle_LYK_digit('\0', &force_load, &old_c, real_c, &try_internal, &lazylynx_follow_link_click);
	    break;
	case LYK_LAZY_LINK_SELECT:
	    handle_LYK_digit('\0', &force_load, &old_c, real_c, &try_internal, &lazylynx_follow_link_select);
	    break;
