...
int lazylynx_follow_link_click(int c, int cur, DocInfo *doc, int *num) {
	return lazylynx_follow_link_base(c, cur, doc, num, 0); //I spent like an hour trying to figure out why it wouldn't follow links, only to find out that actually for no reason if I tell the function not to follow a link then it will and vice versa. Makes sense to me
								//about an hour later it actually does make sense to me. It's cause I'm an idiot and g is the behaviour for going to (as opposed to actually going to) the link.
}
...
int lazylynx_funny_base_52_decode(char *p) {
	...
	while (*p) {
		...
		//normally I would frown at making this search through a string and be O(n) to operate, except that n is bounded by 256, we run this basically three times in the absolute worst case, and this lets people edit their LINK_NUMBERERS strings if they decide that they're too cool to use only/all the letters
...
void lazylynx_generate_numid(char *buffer, int line_number) {
	...
	while (guess_line < line_number) {
		...
		if (i > 29) {
			endwin();
			fprintf(stderr, "Wtf you had more than %d^29 links on your website. This would cause a harmless unchecked buffer overflow in regular lynx - I figured I would lag your program needlessly to 'fix' it. Except that it actually happened to you. idk dude, know this is lazylynx's fault and not regular lynx so don't bother the real devs\n", NUM_NUMBERERS);
			exit(1); //I am sure there is a proper way to do this. But, this idiot really tried to use only 1 character to select all the links in their lynx. Even 2 possible selectors would prevent this. and/or my code bugs out and they can come complain to me to fix it
		}
	}
