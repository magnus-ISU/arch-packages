...
static void add_link_number(HText *text, TextAnchor *a, int save_position)
{
	...
	if (dump_links_inline
	    ...
	    && (link_text = HTAnchor_address(link_dest))) {
	    ...
	    //marker[0]=0;
	    //sprintf(marker, "(%d %d %d %d %d %d)", save_position, (int) saved_lastchar, 0, 0, 0, 0);
	    lazylynx_generate_numid(marker, a->show_number); // there is a bug here  https://www.nice.com.ro/telecomenzi-nice
