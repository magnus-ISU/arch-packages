...
static void initpie(void) {
	...
	XFillArc(dpy, pie.clip, pie.gc, 0, 0, pie.diameter, pie.diameter, 0, 360 * 64);
...
static struct Menu *allocmenu(struct Menu *parent, struct Slice *list, unsigned level) {
	...
	menu->win = XCreateWindow(dpy, rootwin, 0, 0, pie.diameter, pie.diameter, pie.border, CopyFromParent, CopyFromParent, CopyFromParent, CWOverrideRedirect | CWBackPixel | CWBorderPixel | CWEventMask | CWSaveUnder, &swa);
...
static struct Menu *buildmenutree(unsigned level, const char *label, const char *output, char *file) {
	...
	if (prevmenu == NULL) { /* there is no menu yet */
		...
		for (menu = prevmenu, i = level; menu != NULL && i != prevmenu->level; menu = menu->parent, i++)
			;
		if (menu == NULL)
...
static void getmonitor(void) {
	...
	if ((info = XineramaQueryScreens(dpy, &nmons)) != NULL) {
		...
		for (i = 0; i < nmons; i++) {
			if (BETWEEN(mon.cursx, info[i].x_org, info[i].x_org + info[i].width) && BETWEEN(mon.cursy, info[i].y_org, info[i].y_org + info[i].height)) {
				selmon = i;
				break;
			}
		}
...
static void grabpointer(void) {
	...
	for (i = 0; i < 1000; i++) {
		if (XGrabPointer(dpy, rootwin, True, ButtonPressMask, GrabModeAsync, GrabModeAsync, None, None, CurrentTime) == GrabSuccess)
			return;
		nanosleep(&ts, NULL);
	}
...
static void grabkeyboard(void) {
	...
	for (i = 0; i < 1000; i++) {
		if (XGrabKeyboard(dpy, rootwin, True, GrabModeAsync, GrabModeAsync, CurrentTime) == GrabSuccess)
			return;
		nanosleep(&ts, NULL);
	}
...
static void placemenu(struct Menu *menu) {
	struct Slice *slice;
	...
	if (menu->parent == NULL) {
		...
		ret = XTranslateCoordinates(dpy, menu->parent->win, rootwin, menu->caller->x, menu->caller->y, &x, &y, &w1);
		if (ret == False)
...
static void drawslice(struct Menu *menu, struct Slice *slice) {
	...
	XRenderCompositeDoublePoly(dpy, PictOpOver, pie.selbg, slice->picture, XRenderFindStandardFormat(dpy, PictStandardA8), 0, 0, 0, 0, p, npoints, 0);
...
static void drawseparator(Picture picture, struct Menu *menu, struct Slice *slice) {
	...
	XRenderCompositeDoublePoly(dpy, PictOpOver, pie.separator, picture, XRenderFindStandardFormat(dpy, PictStandardA8), 0, 0, 0, 0, p, 4, 0);
}
...
static void drawtriangle(Picture source, Picture picture, struct Menu *menu, struct Slice *slice) {
	...
	XRenderCompositeDoublePoly(dpy, PictOpOver, source, picture, XRenderFindStandardFormat(dpy, PictStandardA8), 0, 0, 0, 0, p, 3, 0);
}
...
static struct Slice *slicecycle(struct Menu *currmenu, int clockwise) {
	...
	slice = NULL;
	if (clockwise) {
		for (lastslice = currmenu->list; lastslice != NULL && lastslice->next != NULL; lastslice = lastslice->next)
			;
...
/* run event loop */
static void run(struct Menu *rootmenu) {
...
while (!XNextEvent(dpy, &ev)) {
	...
	case MotionNotify:
		...
		if (menu == NULL)
			...
			if (menu->selected != slice) { // Magnus start
				menu->selected = slice;
				if (slice->submenu) {
					//currmenu->selected = 0;
					goto selectslice;
	...
	case KeyPress:
		ksym = XkbKeycodeToKeysym(dpy, ev.xkey.keycode, 0, 0);
			...
			} else if ((ksym == XK_Return) && currmenu->selected != NULL) {
				slice = currmenu->selected;
				goto selectslice;
			} else if ((ksym == XK_Escape) && currmenu->parent != NULL) {
