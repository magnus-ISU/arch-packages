...
static void (*handler[LASTEvent])(XEvent *) = {[ButtonPress] = buttonpress, [ClientMessage] = clientmessage, [ConfigureRequest] = configurerequest, [ConfigureNotify] = configurenotify, [DestroyNotify] = destroynotify, [EnterNotify] = enternotify, [Expose] = expose, [FocusIn] = focusin, [KeyPress] = keypress, [MappingNotify] = mappingnotify, [MapRequest] = maprequest, [MotionNotify] = motionnotify, [PropertyNotify] = propertynotify, [UnmapNotify] = unmapnotify};
...
Monitor *createmon(void) {
	...
	m->showbar = showbar;
	m->topbar = topbar;
	m->lt[0] = &layouts[3]; // create fibbonacci layouts as default
...
void drawbar(Monitor *m) {
	...
	if (m->sel && m == selmon)
		drw_setscheme(drw, scheme[SchemeSel]); // keep the top border consistent after the tags
	else
